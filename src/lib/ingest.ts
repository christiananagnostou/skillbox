...
export async function buildIngestPrompt(input: string): Promise<string> {
  ...
  return [
    ...
    "3) Use the schema below as the source of truth (do not search elsewhere).",
    "4) Return strict JSON matching the schema below.",
    "5) Save the JSON to ~/.config/skillbox/tmp/<name>.json or /tmp/skillbox-<name>.json.",
    "6) Or pipe JSON into: cat <file> | skillbox add --ingest -",
    "7) After a successful ingest, delete the JSON file.",
    "",
    DIFY_GUIDANCE,
    "",
    ...skillboxExcerpt,
